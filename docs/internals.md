...
- **キャッシュ**: `${TMPDIR:-/tmp}/claude-statusline-$UID/{git,subscription-v2,usage_spend-v2,resets*}`（**形式を変えたらファイル名の版を上げる** — 据え置くと既存ユーザーだけが旧形式を新コードで読む。v1.74.1） (mkdir 700、親も含めて owner-only) に保存。`CLAUDE_STATUSLINE_CACHE_DIR` で差し替え可 (テスト密閉の seam)。固定の共有パスは共有 Mac で書けず curl storm になるため v1.52.0 でユーザー単位に変更
