...
> **v1 は 2.1.258 で凍結しました（2026-09-03）。** このドキュメントは凍結時点の実装を記述しています。ここに書かれた判断（色・順序・却下記録）は v2 に持ち越す資産で、**機構の記述（US 区切り・形式タグ・背景 subshell・fork ゼロの作法）は bash 3.2 の制約に紐づくもの**なので v2 には引き継ぎません。
