...
- 表示は **2 行**: **Line 4 (このセッション)** = コンテキストバー + % + 分母 → セッション経過時間(単位1つ `41m`/`4h`。**画面で唯一の相対表記** — 時刻ではなく期間なので絶対時刻と混ざらない) → セッションコスト。**Line 5 (アカウント)** = 5hバー + % + リセット時刻(`JST 19:31`、曜日なし・**TZ は行の代表としてここだけ、時刻の前**) → week:% + リセット(`土 16:00`、曜日つき) → モデル別週間枠 → extra-usage実課金
