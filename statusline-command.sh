...
resolve_resets() {
  ...
  # **5h だけ `%Z`（JST 等）を時刻の前に付ける** — 同じ行の時刻は全部このマシンのローカル TZ なので、
  # 行の先頭に 1 回出せば代表になる（4 箇所に付けると同じ語が並ぶだけ）。**前置き**なのは
  # 「ゾーンを宣言してから時刻を並べる」読み順になるため（どちらもユーザー選択 2026-08-17）。
  # Claude Code 自身も専用の TZ 設定を持たず OS のローカル TZ を使うので、表示は一致する。
  # `date` の書式に足すだけなので fork は増えない。
  _memo_reset "$fe" "$c5" "$t5" "%Z %H:%M" _five_txt
