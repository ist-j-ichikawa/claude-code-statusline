...
Line 3: Git (ブランチ [OSC 8 リンク → GitHub tree] + from:親ブランチ + dirty state + ahead/behind + last commit)
...
```
Anthropic(enterprise)  Opus 4.7 (1M context)  high  think  v2.1.141
~/dev/my-project  (+2 dirs)  🌲 from:develop
feature/x  from:main  A3 M2 ?1 ↑2 1h fix: update logic..
...
├── Line 3           Git (ブランチ [OSC 8 リンク → GitHub tree] + from:親ブランチ (reflog) + dirty state + ahead/behind + last commit)、非git時は "no git"
...
| last commit (age + msg)、worktree from、Git branch parent (`from:`) / weekly rate limit | dim | 2 |
