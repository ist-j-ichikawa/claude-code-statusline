...
```
Line 1: プロバイダー + Model + Agent名 + Version + (fork)/(no name)
Line 2: ディレクトリパス(→遷移) + Git (ブランチ + dirty state + ahead/behind + stash + last commit)
Line 3: コンテキストバー
Line 4: レート制限バー + リセット残時間 + 週間制限 + 週間リセット曜日時刻
```

> セッション名は Claude Code 2.1.76+ で右上に組み込み表示されるため、ステータスラインには含みません。未設定時は `(no name)`、フォーク時は `(fork)` を表示します。
...
├── Line 1           プロバイダー + モデル名（Opus=コーラル, Sonnet 4.6=ティール, Sonnet 4.5=アンバー, Haiku=ラベンダー）+ Agent + Version + (fork)/(no name)
├── Line 2           ディレクトリパス(→遷移) + Git ブランチ + dirty state + ahead/behind + stash + last commit
├── Line 3           コンテキストバー
├── Line 4           レート制限バー + リセット残時間 + 週間制限 + リセット曜日時刻
