...
```
Line 1: プロバイダー + Model + Agent名 + Version + (branch)/(no name) + [vim mode]
Line 2: ディレクトリパス(→遷移) + Git (ブランチ + dirty state + ahead/behind + stash + last commit) + 🌲worktree
Line 3: コンテキストバー + セッションコスト+トークン数 + レート制限 (Anthropic)
```
...
```
Anthropic  Opus 4.6 (1M context)  v2.1.85  (no name)  [N]
~/dev/my-project → ~/dev/my-project/src  (main) +3 ~2 ?1 ↑2 1h fix: update logic..
⣿⣿⣄   48%  $0.42 ↑15.2k ↓4.5k  ⣿⣀    16%  2:20  week:9%  金 12:00
```
