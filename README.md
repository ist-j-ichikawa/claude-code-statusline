...
![Built against](https://img.shields.io/badge/Claude_Code-2.1.144-purple)
...
```
Line 1: プロバイダー + Model + effort + think + Agent名 + Version + branch
Line 2: ディレクトリパス + added_dirs (+N dirs) + 🌲worktree from:branch
Line 3: Git ([gh:owner/repo] + ブランチ [OSC 8 リンク → GitHub tree] + from:親ブランチ + dirty state + ahead/behind + last commit)
...
```
Anthropic(enterprise)  Opus 4.7 (1M context)  high  think  v2.1.144
~/dev/my-project  (+2 dirs)  🌲 from:develop
gh:acme/my-project  feature/x  from:main  A3 M2 ?1 ↑2 1h fix: update logic..
...
├── Line 3           Git ([gh:owner/repo (dim, GitHub origin あり時のみ)] + ブランチ [OSC 8 リンク → GitHub tree] + from:親ブランチ (reflog) + dirty state + ahead/behind + last commit)、非git時は "no git"
...
| last commit (age + msg)、worktree from、Git branch parent (`from:`)、Git origin (`gh:owner/repo`) / weekly rate limit | dim | 2 |
