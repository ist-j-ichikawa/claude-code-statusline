...
```
Line 1: プロバイダー + Model + Agent名 + Version + (branch) + [vim mode]
Line 2: ディレクトリパス (+N dirs) + Git (ブランチ + dirty state + ahead/behind + last commit) + 🌲worktree from:branch
Line 3: 5hレート制限 + コンテキストバー + ↑入力トークン + ↓出力トークン + セッションコスト + weeklyレート制限
```
...
```
Anthropic(enterprise)  Opus 4.6 (1M context)  v2.1.92  [N]
~/dev/my-project (+2 dirs)  (main) A3 M2 ?1 ↑2 1h fix: update logic..  🌲 from:develop
⣿⣀    16%  2:20  ⣿⣿⣄   48%  ↑15.2k ↓4.5k  $0.42  week:9%  金 12:00
```
