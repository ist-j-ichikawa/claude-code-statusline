...
- **Opus 5.5 に専用の配色を付けました**（藍 → 灰みの藤色 → 橙）。発表ページのシェア用画像の「軌道から見た夜明けの地平線」から取っています。これまでは Opus 5 と同じ配色でした。**末尾の橙は Opus 5 と共通**なので同じ系譜だと読め、世代は先頭の色で見分けられます。公式の単色はまだ発表されていないので暫定です
- Built against を Claude Code **2.1.281** に追従しました（2.1.273 → 281 の CHANGELOG 8 節・648 行と、2.1.278 / 2.1.281 のバイナリを照合）。**ステータスラインが受け取る JSON のキーは 1 つも増減していません**
  - **Claude Code 2.1.281 から、複数行のステータスラインは前の行の色（とリンク）を次の行の頭に持ち越して描かれます**（上流の CHANGELOG には載っていない変更）。このスクリプトは各行を色のリセットで閉じているので見た目は変わりませんが、行末で色を閉じることが本体側の前提になったため、テストで固定しました
- README の Line 1 の説明を直しました。**Claude Code の版は常に出ています**（追いついていれば薄く、遅れていれば赤）。「遅れている間だけ出る」と書いていたのは誤りでした

### Fixed

- **Keychain の項目を引くときのアカウント名を Claude Code 本体と同じ作り方にしました。** 本体はユーザー名が英数字と `. _ -` 以外を含むと `claude-code-user` という名前で保存しますが、こちらはその判定を持っていなかったため、**そういうユーザー名の環境ではプラン名が出ませんでした**。あわせて、アカウント名を付けずに引く経路を無くしました（本体は必ず付けて保存するので、付けずに引くと別の項目に当たりえます）

## [2.3.0] - 2026-09-16

### Changed

- **モデル別の週間枠が 0% のときは出さないようにしました**（アカウント週間枠と同じ扱い）。`0%` は「今週そのモデルを 1 度も使っていない」という意味なので、別のモデルを常用していると**空のバーが 3 行目の右端に居座り続けます**。3 行目は左から優先度順に並んでいて右端は狭い窓で最初に切られる位置なので、常に同じ値の要素がそこを占めるのは桁の無駄でした
  - **`5h` は 0% でも出します**（左端の一目確認用）。隠すのは週間の枠だけで、この版でその規則をモデル別枠にも適用しました
  - 枠が複数あるときは**0% のものだけが落ちます**（1 件でも 0% があると行ごと消える、という実装にはなっていません）

## [2.2.0] - 2026-09-16

### Changed

- **5h / 週間 / モデル別の枠が 90% を超えたら、その要素をまるごと赤くするようにしました。** これまで値で色が変わるのはコンテキストだけで、枠は 91% でも平常色（`5h` はタン、`week` はその dim、モデル別枠はモデル色）のままでした。**コンテキストと同じ 90% を閾値**にしてあるので、3 行目の閾値は 1 つです
  - **バーだけを赤くする形は採っていません** — ラベルに識別色を残してバーに別の色系統を当てると「1 つの要素が 2 つに割れて見える」ため、**識別色を赤に差し替えます**。`week` は dim も外します（尽きかけている枠は二次情報ではありません）。モデル別枠はモデル色のグラデーションを捨てて赤 1 色になります
- Built against を Claude Code **2.1.273** に追従しました（2.1.270 → 273 を教典 3 つで diff）。**実質差分はゼロ** — CHANGELOG は差分なし、`/statusline` のプロンプトは minify 名だけ、公開 docs は表記ゆれ（`Claude.ai` → `claude.ai`）と説明文の言い換えだけで、**受け取る JSON の契約は動いていません**
...
- **ゼロから作り直した 3 行の実装に置き換えました。** 5 行の `1.x` とは別実装で、**設定のパスは同じ**（`statusline-command.sh`）なので `git pull` すればそのまま切り替わります。**表示が変わるので、1.x のままにしたい場合は `v1.90.0` を checkout してください**
  - **ファイルは 1 本だけ**になりました。`lib.sh` は取り込み、`subagent-statusline-command.sh` / `install.sh` / `test.bats` / `docs/internals.md` は削除しました（`subagentStatusLine` の行は無くなりました）
  - **1 描画で外に出るプロセスは `jq` 1 個 + `git` 1 個**（1.x は git を約 10 回呼んで背景 subshell と 5 秒キャッシュで隠していました）。`date` / `stat` / `md5` は 0 個です
  - **キャッシュは 1 個だけ**残しました（プラン名とモデル別週間枠は `stdin` に来ないため）。レコードは**自己記述の key-value 行**で、未知のキーは読み飛ばし、無いキーは既定値に倒れます — **項目を足しても既存のレコードが無効化されない**ので、版を上げたときに全セッションが同時に取り直す事故が起きません。鮮度は出所ごとに持ちます
  - **出さないと決めたものが増えました。** 組み込みの UI が常時見せているもの（PR の状態 / セッション名 / vim モード / 変更行数）と、後から取り戻せる累積値（プロンプトキャッシュの hit_ratio 等）は落としました。判断の基準は「一過性か、決断のトリガーか」の 2 つだけです
  - **足したもの**: プロンプトキャッシュが cold になった瞬間とその原因、fast モード、モデル別週間枠のリセット時刻、5h/週間/モデル別枠の braille ゲージ、`timeZone` 追従、cross-session messaging の宛名

## [1.90.0] - 2026-09-03

### Changed

- **v1 をここで凍結しました。** Built against は Claude Code **2.1.258** で止め、以後は定期的な上流追従も機能追加も行いません。**利用者に必要な操作はありません**。ただし **1.90.0 に上げると v1.89.0 の不具合修正 2 件で表示が変わります**（サブエージェント行のモデル名から日付が落ちる / 説明文の制御文字で行が崩れなくなる）。**それ以降は表示が変わることはありません**
  - **凍結する理由は機構の負債**です。表示の判断（何をどの色でどの順に出すか）は資産ですが、それを支える機構が bash 3.2 の制約に強く縛られていました — 単一 `jq` + `@sh` + `eval` で 34 変数を抽出、US (`0x1f`) 区切りの平文レコード 41 箇所、キャッシュの形式タグ 4 個と背景 subshell、手書きの JSON unescape。実装 1,081 行に対して `test.bats` が 4,079 行あり、その相当部分が **bash の罠を守るため**のものです
  - **キャッシュ機構の本当の理由は「git を約 10 回呼んでいた」こと**でした（1 描画ぶんの facts 収集で `branch --show-current` / `rev-parse` ×3 / `remote get-url` / `diff --numstat` / `ls-files --others` / `diff --diff-filter=U` + `grep` / `rev-list --count` ×2 / `log -1`）。10 回 × 9〜17ms は同期で払えないので背景 subshell + 5 秒キャッシュにしていて、その代償として**表示は最大 5 秒古く、初回の描画はブランチ名だけ**でした。`git status --porcelain=v2 -b` 1 発に畳めば 4 プロセス並列で 32〜35ms（実測）になり、**キャッシュ無しでより正確に**出せます
  - **凍結を解く条件**: ① 不具合の報告が来たとき ② 要望が出たとき ③ Claude Code の変更で表示が壊れたとき。どれも「そのとき考える」で、**定期的な追従はしません**
  - ゼロから作り直す **v2** を同じリポジトリで検討しています（`bun` 実装・3 行・キャッシュ全廃の方向。**どこまで `stdin` だけで完結させるかは未決**）。**v1 のファイルには触りません**し、v1 の設定を変える必要も生じません

## [1.89.0] - 2026-09-02

### Fixed

- **サブエージェント行のモデル名から日付を落とすようにしました。** `claude-haiku-4-5-20251001` のような日付つき id が `Haiku 4.5.20251001` と表示されていて、1 行目の `Haiku 4.5` と同じモデルが同じ画面に 2 通りの名前で並んでいました。**落とすのは 8 桁ちょうどの日付だけ**です（「末尾の数字」を落とす形にすると `opus-5` の版まで食います）
- **サブエージェントの説明文に制御文字が混ざっても行が崩れないようにしました。** 従来は改行とタブだけを空白化していたため、US (`0x1f`) を含む説明が来ると抽出行が割れて、**モデル名が「状態」の位置に入り生の id が黄色で出る**状態になりました（実測）。エスケープ文字も同時に落ちるので、**説明文から色を差し込まれることもなくなります**（行に色を付けるのはステータスライン側だけになります）。**クラス名 `[[:cntrl:]]` で書いているのは escape が要らないから**です — 正規表現側のエスケープとして `\u0001-\u001f` を渡すと jq の正規表現エンジンが字面で解釈して別の文字を潰し（実測）、jq の文字列エスケープとして書くと今度は**プログラムの中に生の制御文字が埋まって `grep` で見えなくなります**

### Changed

- **サブエージェント行の描画を 17.4ms → 11.3ms にしました**（実測。2 タスクの入力で 30 回平均）。① stdin を変数に読まず `jq` に直接渡す ② 出力の JSON 化を `jq` から bash の文字列置換に移す、の 2 つで、**起動する外部プロセスは抽出の `jq` 1 個だけ**になりました（bash 3.2 が here-string のたびに作る一時ファイル 2 個も消えています）。**出力されるバイト列は変わりません**（エスケープ文字は従来と同じ `\u001b` 形式）。`jq` の起動回数はテストで 1 回に pin してあります
- **README を最新化・整理しました。** 表の中に紛れ込んでいた注記を表の前に出し、表と本文がくっついていた箇所に空行を入れ、**相対時刻のままだった Line 3 の例**（`master  0m initial commit`）を現行の絶対時刻表記に直して「origin 未設定」の説明の直後に移しました。宛名の説明は 5 段落から 3 段落に畳み、詳細は `docs/internals.md` の「宛名」節に寄せています
- **参考として載せているフッターの GitHub リンクバッジ設定（`footerLinksRegexes`）を見直しました。** これは**ステータスラインの機能ではなく** Claude Code 本体の別機能ですが、README に「動作を確認した設定」として載せている以上は実物と合っている必要があるため、2.1.258 の実装を読み直して更新しました
  - **リポジトリ名の文字クラスを緩めました** — `.github` や `_priv` のように記号で始まるリポジトリ名が拾えていませんでした（アカウント名側は英数字とハイフンだけなので、そちらは絞ったままにしています）
  - **リリースタグ用のパターンを足しました**（`release v1.89.0` のバッジになります）。tag の末尾を `[\w.+-]*\w` にしてあるので、文末の句点を URL に取り込みません
  - **ベアな `owner/repo#123` の遷移先を `/issues/` に戻しました**（README に載せていた形。GitHub では PR も issue 番号を共有するので、こちらに送れば PR でも解決します）
  - 走査範囲（直前のユーザー入力より後の出力だけ・ツール結果は末尾 8 KB・全体で末尾 64 KB / 256 メッセージ）、**ラベルは 28 文字で切られる**、同じ URL は 1 個に畳まれる、URL テンプレートはリテラルのオリジンが必要、といった実装上の上限を注記に追加しました
- **`docs/internals.md` に「stdin の `session_name` は宛名ではない」ことを明記しました。** 2.1.258 の payload 構築コードでは `session_name` が `customTitle ?? aiTitle`（= 右上に出る表示名）なので、**`sessions/*.json` の走査をこのフィールドに置き換える最適化は成立しません**。置き換えると「送れない名前を送信先として見せる」= `SendMessage` の誤配を新設します

## [1.88.0] - 2026-09-02

### Added

- **`settings.json` の `timeFormat` と `timeZone` に追従するようにしました**（Claude Code 2.1.257 で増えた設定）。`/config` の Time format で 12 時間表記や UTC を選ぶと、5h と週間のリセット時刻もそれに合わせて出ます。**Claude Code 本体の時計と表記がずれないようにするためだけの機能**で、ゾーン名を表示しない方針は変わりません
  - 未設定 / `auto` は従来どおり `19:31` / `土 16:00`。`"12-hour"` は `7:31 PM`、`"24-hour-utc"` は `10:31Z`、`%` を含む値は strftime パターンとしてそのまま使います。**`"24-hour-utc"` は `timeZone` より優先されます**（本体と同じ挙動）
  - **`auto` の locale までは追いません** — 本体の `auto` は locale の時計方式に従いますが、`date` に「この locale の時刻書式」を安全に出させる指定が無いためです。12 時間表記の locale で本体が `7:31 PM` を出す場合は `"12-hour"` を明示してください
  - **読むのはユーザー設定** (`~/.claude/settings.json`) だけです。プロジェクト側に書いた場合は反映されず従来表記のままになります（誤った書式を出すことはありません）
  - **README で `"timeFormat": "24-hour"` を推奨に加えました。** 本体の既定 `"auto"` は端末の locale 任せなので、`LANG=en_US.UTF-8` のような環境では**本体が 12 時間表記・ステータスラインが 24 時間表記**という食い違いが既定で起きます。`install.sh` はこのキーを書きません（時刻の好みは人によるので、`refreshInterval` / `hideVimModeIndicator` のような不具合の回避とは性質が違います）
  - タイムゾーン・書式・**ロケール**が変わったらキャッシュを取り直します。覚えているのは整形済みの文字列なので、これを怠ると（例えば `LANG` を英語から日本語に変えたとき）**週間の枠だけ曜日が `Sat` のまま最大 1 週間残ります**
  - **設定は既存の 1 回の `jq` に相乗りさせているので、起動するプロセスは増えていません。** 生文字列として渡して開けるので、**`settings.json` が壊れていてもステータスラインは空白になりません**（設定なしとして扱われます）
  - 不正なタイムゾーン名は無視してこのマシンのゾーンに戻します。**libc は不正な `TZ` を黙って UTC にする**ので、これを落とさないと「UTC の時刻をローカルだと思って読む」誤読になります。実在チェックだけでは足りず（`America` のようなディレクトリや `zone.tab` も通ってしまう）、ゾーンファイルの中身まで見て判定します
  - 書式に改行やタブを混ぜても**5 行のまま**です（`%n` を直接書いた場合だけでなく、`%%nn` や `%En` のような回り込みも塞いでいます）。`settings.json` が JSON として有効でも object でない場合（`[]` や `5`）にステータスラインが壊れる経路も塞ぎました

### Changed

- **Claude Fable 5.1 の色を、5.1 の発表記事の図版（Venus / Magellan のレーダー画像）由来のスイープに変えました。** brick → tan → amber → cream の 1 回スイープ（`95` → `137` → `214` → `187`）です
  - **Fable 5 の蝶標本パレットはそのまま残ります** — 旧モデルを選んだ画面の見た目は変わりません
  - 版を持たない `Fable`（Line 5 のモデル別週間枠は `/usage` から版なしの名前で来ます）は 5.1 側の色になります。既定モデルが 5.1 なので、Line 1 と Line 5 の色が揃うのはこの向きだけです
  - 図版は Fable 5.1 自身が作った成果の説明図です（NASA Magellan のレーダー画像から金星の 1/3 の標高マップを作り直した話）。Fable 5 の蝶標本のような「モデルの識別画像」は 5.1 には無く、発表ページはヒーロー画像を持ちません
  - 公式のブランド色は 5 も 5.1 も未発表のままなので、どちらも暫定色です。**アートボードの実測値そのままにはしていません** — 最も暗い色は 256 色に落とすと純グレーで黒背景に沈み、最も明るい色は白に近くて明るいテーマで飛ぶため、相対輝度（`0.2126R + 0.7152G + 0.0722B`）がほぼ等間隔になるよう引き直しました。すぐ左に出る `Anthropic` ラベルと同じ色（`180`）は使っていません
- Built against を Claude Code 2.1.258 に追従しました（`/check-claude-code-update` で 2.1.252 / 2.1.257 / 2.1.258 を分析。2.1.253〜256 は上流の欠番）。**ステータスラインが受け取る JSON のフィールドは 1 つも増減していません**（公式 docs の項目一覧と、2.1.252 / 2.1.258 のバイナリに埋まっている payload のキー一覧の両方で照合）。サブエージェント行の契約（`tasks[]`）も 13 項目で変化なし
  - **Fable 5.1 は 2.1.257 で Fable の既定モデルになりました。** 色の分岐そのものは追加作業なしで当たります（`claude-fable-5-1` も `Fable 5.1` も同じ正規形に畳まれるため）
- **4 行目のセッション経過時間の説明を正確にしました**（表示は変わりません）。`cost.total_duration_ms` の実体は「セッションを開いてからの壁時計」で**離席していた時間も含みます**。「Claude が何時間動いていたか」ではないため、コードのコメントと `docs/internals.md` の記述を直しました（実働時間は `cost.total_api_duration_ms` 側）

### Fixed

- **`CLAUDE_CONFIG_DIR` で複数アカウントを併用したとき、別アカウントの情報が表示される**のを直しました。実課金額とモデル別週間枠（最大 5 分）、**プラン名とレート枠**（`Anthropic(Max 20x)`、最大 1 時間）の 3 つが対象で、キャッシュのファイル名に設定ディレクトリを混ぜます（リセット時刻のメモは以前からこの形でした）。併せて、混ざったレコードが互いに「形式が違う」と判定し合って**毎回の描画で `/usage` を叩き直す**状態にもならなくなります
- 取得に失敗している最中に時刻の書式を変えると、**毎回の描画で `/usage` を叩き続ける**経路がありました（Wi-Fi が切れている間に `/config` で Time format を変えると踏みます）。この場合はレコードを書き直して収束させます
- テストが**開発者自身の `~/.claude/settings.json` を読んでいた**ため、`timeFormat` を設定すると 4 件落ちる状態でした。時刻や枠を検証するテストは空の設定ディレクトリに隔離します（出荷物の挙動は変わりません）
- **毎月 1〜9 日だけテストが 1 件落ちていた**のを直しました。`dirty state: 0の側を出さないこと` が「削除行が 0 なら `-0` を出さない」を裸の部分一致で見ていたため、同じ行に並ぶ最終コミットの日付（`09-02T10:24` の `9-02`）に誤ヒットしていました。区切りのスペースまで含めて判定します

## [1.87.0] - 2026-08-31

### Added

- **プロンプトキャッシュの状態を 4 行目の末尾に出すようにしました**（Claude Code 2.1.251 で増えた `prompt_cache`）

  ```
  ⣿⣶   60%/1M  3h  $4.83  prompt_cache:warm hit_ratio:91%
  ```

  - `warm` はキャッシュが期限内、`cold` なら次のリクエストが会話を焼き直します（コストが跳ねる理由になります）
  - `hit_ratio` は入力トークンのうちキャッシュから読めた割合です。下がっていれば、何かがキャッシュの前提を壊しています
  - **この 2 つだけ出します。** Claude Code は他に 10 項目（リクエスト数・書き込みトークン・期限時刻など）を渡してきますが、累計や固定値で見ても操作が変わらず、金額は既に `$` で見えています。全項目は `/usage` の `Prompt cache (main)` 行で見られます
  - `prompt_cache` はセッション最初の応答まで来ないので、それまでは何も出ません（**行数は 5 行のまま**変わりません）

### Changed

- Built against Claude Code 2.1.251
  - 2.1.251 の 1 版だけを確認しました（2.1.252 はまだ出ていません）。**stdin に新しいデータが 2 つ増えた版**で、上の `prompt_cache` がその 1 つです
  - もう 1 つの `rate_limits.spend_limit`（Claude apps gateway の支出上限）は**出しません**。この認証方式のときだけ届くもので、こちらの環境では確認できないためです
  - Claude Code 側で、Opus 5 の effort を `xhigh`/`max` にしたまま思考を切ると、実際のリクエストは `high` で送られるようになりました（2.1.251 の修正）。**1 行目の effort 表示は設定した値のままなので、この組み合わせでは画面と実際が 1 段ずれます** — 実際に送られた値は Claude Code が statusline に渡していないので、こちらからは直せません

### Fixed

- **テストが日本語ロケールの環境だけで 1 本落ちていたのを直しました**（出荷物には影響しません）。リセット時刻の曜日を比べるテストで、スクリプト側は空の環境で走らせるため `Sat`、テスト側は `LANG=ja_JP.UTF-8` のため `土` になり、食い違っていました

## [1.86.0] - 2026-08-28

### Changed

- Built against Claude Code 2.1.250
  - 2.1.246 / 2.1.247 / 2.1.248 / 2.1.250 の 4 版を確認しました（2.1.249 は欠番）。**表示は変わりません** — Claude Code が渡してくるデータの形は 2.1.245 から 1 つも増減していませんでした（メインの 43 項目・サブエージェント行の 13 項目とも一致）
  - **このセッションのコストと経過時間が、エージェント一覧を行き来したあとに 0 に戻る不具合が Claude Code 側で直りました**（2.1.246）。4 行目の `$4.83` と `3h` がそのまま正しくなります
  - **PR のレビュー状態（3 行目の `approved` など）は、以前より古い値が出ることがあります**。Claude Code が GitHub への問い合わせを間引くようになったためです（2.1.247 / 2.1.248）。ただし push や `gh pr` を実行したときは即座に更新されます
  - 宛名（1 行目のセッション名）のもとになるセッション間メッセージが、Bedrock / Vertex AI / Foundry でも使えるようになりました（2.1.248）。**これらのプロバイダーでも宛名が出るようになります**

## [1.85.0] - 2026-08-25

### Changed

- **追加課金の表示を `extra:$12.34` から `credits:$12.34` に変えました**。Claude Code 自身が 2.1.144 で「extra usage」を「usage credits」に改名し（`/extra-usage` → `/usage-credits`）、以後の画面表記はすべてそちらになっているので、こちらの語彙もそろえます。出る条件と色（金色の太字）は変わりません
  - 2 文字ぶん長くなります。この行の他の要素は変わりません

### Fixed

- **モデル別の週間制限で、使用率 0% の枠を出さないようにしました**。`Sonnet 5:0% 土 16:00` のように 24 桁を使って「今週まだ使っていない」ことだけを伝え、リセット時刻まで連れてくる表示になっていました
  - 同じ行にあるアカウント全体の週間制限（`week:`）は以前から 0% のとき出していなかったので、**週間の枠どうしで扱いが揃っていなかった**のを直した形です。5 時間の枠は 0% でも出します（行の左端で今の状況をひと目で見るための表示なので、そこは変えていません）
  - Claude Code 2.1.236 から「まだ何も使っていない枠」も返ってくるようになったので、この表示に当たる機会は今後増えます

## [1.84.0] - 2026-08-25

### Changed

- Built against Claude Code 2.1.245
  - 2.1.236〜2.1.245 の 8 版を確認しました。**表示は変わりません**（stdin で渡ってくるデータの形は、下の 1 点を除いて 2.1.235 と同一でした）
  - レート制限の枠は、**リセット時刻を過ぎると Claude Code 側が送るのをやめる**ようになりました（2.1.243）。これまでは古い使用率がそのまま残っていて、こちらからは古いと判別できませんでした。今は枠ごと表示が消えるので、**古い数字を見せてしまう経路が無くなります**
  - Claude Code はリセット時刻ちょうどにステータスラインを描き直すようにもなりました。**5h の枠と `week:` は次の応答を待たずに切り替わります**が、モデル別の枠（`Fable:8%` 等）はこちらが 5 分間キャッシュしている値なので、切り替わりまで最大 5 分ずれます
  - 使用クレジットの請求額は、Team / Enterprise でも 0 円のうちは出しません（2.1.236 で Claude Code 側が「まだ何も使っていない枠」も返すようになりましたが、こちらは 0 円のときは要素ごと出さない扱いのままです）

## [1.83.0] - 2026-08-21

### Added

- **GitLab のリポジトリでも 3 行目にリポジトリ名とブランチのリンクが出るようになった**。GitHub の `gh:` と並べて GitLab は `gl:` を付け、ブランチ名をクリックすると GitLab のツリー表示が開きます。これまで GitLab では 3 行目がブランチ名から始まり、リンクもありませんでした
  - マージリクエストのレビュー状態（`draft` / `approved` / `pending`）はこれまでも出ていました。番号は Claude Code 自身のフッターが `MR !42` として出すので、こちらには出しません

### Fixed

- **設定ディレクトリを切り替えて使っているセッションで、別のアカウントのプラン名と課金額を出しうる問題を直した**。認証情報の保管場所は設定ディレクトリごとに分かれているのに、常に既定の場所を見ていたためです。取り違える経路を閉じ、読めないときは無表示になります
- **origin の URL に `user@` が入っているリポジトリで、ブランチ名がリンクにならない問題を直した**
- **名前を変更したセッションで、宛名として過去の名前を出しうる形を防いだ**。今の並びでは正しい名前が出ていましたが、並び順だけが頼りだったため、取り違えの経路そのものを閉じました
- **認証情報を読むときに、利用者アカウントも指定して引くようにした**。同じ名前の項目が 2 つあるキーチェーン（別のマシンから同期した場合など）で、別アカウントのプラン名と課金額を出しうる経路が残っていました
- **origin の URL が `/` で終わっているリポジトリで、3 行目のリポジトリ名に余分な `/` が付く問題を直した**。「続きは真上の行」を意味する記号と紛れ、リンク先も壊れていました
- **対応していないホスト（Bitbucket や自前サーバーなど）で、GitHub のものとして `gh:` を出しうる形を閉じた**。略号とリンクの形はサービスごとに違うので、判別できないときは要素ごと出しません

### Changed

- Built against Claude Code 2.1.235

## [1.82.0] - 2026-08-18

### Changed

- **描画がもう少し速くなった**（同じ条件で新旧を交互に 5 回測った中央値で 34.0ms → 31.0ms）。表示は変わりません
  - 現在時刻を取るためのコマンド起動をやめ、**すでに 1 回だけ動いている JSON 解析から受け取る**ようにした。これで 1 描画あたりの外部コマンドは 3 個（JSON 解析・ハッシュ・ファイル時刻）になり、時刻取得ぶんが 0 個になります
  - 受け取れなかったときは従来どおりコマンドで取り直すので、表示が崩れる経路はありません

## [1.81.0] - 2026-08-18

### Changed

- **描画が約 23% 速くなった**（同じ条件で新旧を交互に 4 回測って 49.0ms → 37.5ms。絶対値は同時に動いている他の処理で 34〜52ms の幅があるため、差のほうが再現します）。表示は変わりません
  - キャッシュの鮮度を見るための `stat` を、3 つのキャッシュぶん 1 回にまとめた（-6.7ms）。ファイル名も一緒に取って突き合わせるので、片方が無いときに別のキャッシュの時刻を読む取り違えが起きない
  - Git 情報を分解するために一時ファイルを作っていたのをやめた（-3.0ms）
  - 受け取った JSON をいったん変数に入れてから解析していたのをやめ、そのまま解析するようにした（ここでも一時ファイルが 1 つ消える）

## [1.80.0] - 2026-08-17

### Fixed

- **非 git ディレクトリで、Git 情報の取得が毎回バックグラウンドで走っていた**問題を修正。空のキャッシュを「形式が違う」と見なしてしまい、5 秒の抑止が効かずに毎描画で取得し直していた（表示は正常なので気づけない）
- **追加課金の取得が失敗したとき、エラー応答で良いキャッシュを上書きしていた**問題を修正。API が 401/429 などの JSON を返すと「取得成功・課金 0」と区別できず、`extra:$` とモデル別の週間枠が 5 分消えていた。判定を「応答が空か」から「必要な項目が入っているか」に変更
- **`git am` の中断中を「rebase 中」と表示していた**問題を修正（`am 2/5` と出す）。`git rebase --abort` を打とうとして「am には無い」と気づく手戻りを防ぐ
- **バージョンにゼロ埋め（`2.1.08` 等）が含まれると、毎描画でエラー出力が漏れていた**問題を修正（8 進数として解釈されていた）
- **レート枠が 3 桁（`100x`）だと表示されなかった**問題を修正。「末尾が `Nx` の形かだけを見る」という約束に対して、桁数の上限が残っていた

### Changed

- **変更行数の色を GitHub の diff 色（ダークモード）に合わせた**。追加 `--fgColor-success #3fb950` / 削除 `--fgColor-danger #f85149` の 256 色最近傍。削除色は v1.75.0 でスクリーンショットから採色していたが、あれは背景と混ざった値で本来より暗く濁っていた
- **ダークテーマ推奨であることを明記**（README / docs）。色は暗い背景でのコントラストを基準に選んでおり、明るい背景では一部の要素が読めない（白地に対して 1.0〜1.6:1 の実測）。ライトテーマ用の配色は用意しない — 端末の地色を知る手段が無いため、両対応にすると暗い背景でのコントラストを捨てることになる

## [1.79.0] - 2026-08-17

### Changed

- **タイムゾーン名の表示をやめた**（v1.78.0 で入れた `JST 19:31` → `19:31`）。画面の時刻は**例外なくこのマシンのローカルタイムゾーン**なので、ゾーン名を毎回出しても情報が増えなかった。どのゾーンで表示されるかは README と docs に書いてある（1 度読めば済むことを毎描画に置かない）
  - Claude Code 自身も専用のタイムゾーン設定を持たず OS のゾーンを使うため、両者の表示が食い違うことも無い
  - 再検討する条件は「複数のゾーンの時刻を並べる要素が出たとき」

## [1.78.0] - 2026-08-17

### Changed

- **最終コミットの時刻を絶対表記にした**（`19m` → `08-17T13:13`）。区切りは ISO 8601 風（`-` と `T`）。「何分前か」ではなく「何時のコミットか」が知りたいという要望。180 日より古いコミットは `2025-08-17`（年つき・時刻なし — 古いコミットに分単位の意味は無く、年が分からないほうが困る）
  - これで**画面上の時刻は絶対表記に揃った**。相対表記はセッション経過（`4h`）だけで、これは時刻ではなく期間なので混ざらない
- **5h リセットにタイムゾーンを添えた**（`JST 19:31`）。同じ行の時刻はすべてこのマシンのローカルゾーンなので、**行の代表として 1 回**だけ、**時刻の前**に置く（ゾーンを宣言してから時刻を並べる読み順）（4 箇所に付けても情報は増えない）。Claude Code 自身も専用のタイムゾーン設定を持たず OS のゾーンを使うので、表示は一致する。時刻変換の書式に足すだけなので処理は増えていない
