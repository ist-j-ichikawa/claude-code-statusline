...
- **キャッシュの形式判定をファイル名からレコードの中身へ移した**。v1.74.1 で付けたファイル名の版（`-v2`）をやめ、**先頭に「フィールド一覧」をタグとして書く**形にした
  - 版番号を名前に持たせるのをやめた理由: ① 製品のバージョン（`version-1.76.0`）と紛らわしい ② Git 情報は `-v4`、他は `-v2` と**意味のない序列**が見える ③ **番号を上げるのは手作業で、実際に忘れて出荷した**（それが v1.74.0 の不具合） ④ 古いファイルが同じ場所に並んで、どれが現行か名前から分からない
  - タグはフィールド一覧そのものなので、**形式を変える人は必ずそれを編集する**（番号より忘れにくい）。名前が変わらないので**古いファイルが残らない**
  - タグが合わないキャッシュは、値を捨てるだけでなく**「古い」と同じ扱いにして即座に取り直す**。以前の方式では、形式が変わった直後に「新しいけれど読めないファイル」が寿命いっぱい居座って表示が欠けていた
  - 4 つのキャッシュすべて（Git 情報・契約種別・課金と週間枠・リセット時刻の記憶）に適用。テストは「使われないこと」と「取り直されること」を別々に確かめる

## [1.76.0] - 2026-08-17

### Changed

- **output style を常に表示するようにした**（`default` のときも `default` と出る）。v1.74.0 では `default` 以外のときだけ出していたが、今どの output style で動いているかは `/output-style` を開かないと分からないので、常に見えるほうを選んだ
  - **`default` だけ弱め表示**にしている。既定値は「特に設定していない」を示すもので、`no git` などと同じ扱い。非既定は白で出るので「既定と違う」ことは色の差で読める

## [1.75.0] - 2026-08-17

### Changed

- **変更行数と ahead/behind の色を端末テーマに依存しない色に変えた**。これまで標準の「緑」「赤」を使っていたが、**その 2 色は端末のテーマが自由に置き換えられる**ため、実際の環境では緑がオリーブ（#b5bf70）、赤がレンガ色（#c36c68）に化けて「緑と赤」に見えていなかった。256 色を明示して、どのテーマでも同じ見え方になるようにした（値は Claude Desktop の code 画面から採色）
  - **注意を促す赤とは分けた** — コンフリクト `!N` / detached HEAD / コンテキスト 90% 超 / 最新から遅れた版は「問題」を示す赤、`+N -N ↑N ↓N` は「量」を示す色。役割が違うので同じ赤を共有しない

## [1.74.1] - 2026-08-17

### Fixed

- **v1.74.0 に上げた直後、Max プランのレート枠（`5x` / `20x`）が最大 1 時間表示されなかった**問題を修正。v1.74.0 でサブスク情報のキャッシュの中身を「契約種別だけ」から「契約種別＋レート枠」に変えたのに、ファイル名を据え置いていたため、**v1.73.0 が書いた古いキャッシュを新しいコードが読んで**枠が空になっていた（`Anthropic(Max 5x)` が `Anthropic(Max)` になる）。キャッシュの寿命が切れれば自然に直るが、**アップグレードした人は全員通る経路**だった
  - キャッシュのファイル名に版を付けた（`subscription-v2` / `usage_spend-v2`）。Git 情報のキャッシュが以前から採っている作法に揃えたもので、古いファイルは読まれずそのまま残る（`TMPDIR` は OS が定期的に掃除する）
  - モデル別の週間枠も同じ理由で最大 5 分欠けていた（こちらは寿命が短く気づきにくかった）ので、同時に修正
  - **テストがこの種の欠陥を構造的に見られなかった**点も塞いだ。テストは毎回空のキャッシュから始まるので「古いキャッシュがある状態で新しいコードが動く」経路を一度も通らず、**新規インストールでは正しく既存ユーザーだけが踏む**欠陥が緑のまま通っていた。旧ファイル名を置いて「読まれないこと」を確かめるテストを 2 件と、キャッシュの版の付け忘れを禁じるメタテストを追加
  - 表示側のロジック（レート枠の値を列挙せず末尾が `Nx` かだけを見る形）は正しく動いていたので変えていない

## [1.74.0] - 2026-08-17

### Changed

- **Line 3 の変更表示を「変更行数」にした**（`A3 M6 ?1` → `+470 -105`）。Claude Desktop の code 画面と同じ単位・同じ色（`+` 緑 / `-` 赤）
  - staged と unstaged を合算する（Desktop の「ワーキングツリー」表示と同じ範囲）。ステージ済みかどうかの区別は表示されなくなる
  - 未追跡ファイルの行数は追加側に畳む（Desktop も未追跡を「追加」として扱い、専用の記号を持たない）。`?N` の件数表示は廃止
  - コンフリクト（`!2`、赤）は Desktop の状態一覧に無いので、独立した表示として残している（マージ中は最優先の情報のため）
  - 追加だけ・削除だけの作業では `-0` / `+0` を出さない
  - バイナリファイルは行数を持たないので数えない
  - **未追跡ファイルが 500 件を超えるリポジトリでは未追跡の行を数えない**。数えるコストは未追跡の総バイト数に比例するので、`node_modules` などを無視設定に入れていないリポジトリ（実測 30,000 件で 5.17 秒）では Git 情報の更新が間に合わず、更新のたびに全走査をやり直す状態になる。途中まで数えた合計は「間違った数」なので、その場合は未追跡ぶんを出さない
- **モデル別週間制限の使用率を通常輝度にした**（`Fable:39%`）。弱め表示だと数値が沈んで読めなかった
- **金額を金額らしくした**。セッションコストをブロンズ、extra-usage の実課金を明るい金＋太字にして、**同じ金色系の明度差で「参考値 / 実際に請求される額」の序列**を付けた。コストが無色だったのは extra と隣接して混同するためで、行分割で別行になりその前提が消えた
- **vim mode バッジの色を vim 側の流儀に合わせた**。INSERT を緑から**青**へ、VISUAL を**橙**へ。緑は lualine の gruvbox_dark と vim-airline 既定では NORMAL / COMMAND の色で、INSERT に使うとモードを誤読させる（緑を INSERT に使うのは lightline 系の流儀）
- **effort をレベルごとの色にした**（Claude Code 自身の `/effort` ピッカー準拠）。`low` gold → `medium` green → `high` 薄紫 → `xhigh` 濃紫 → `max` は多色（紫→桃→橙）。単色だった頃は `high` と `max` を色で区別できなかった。未知のレベルは既定色に落ちる。subagent 行も同じランプを使う
- **Line 4 をスコープで 2 行に分割**（通常 5 行になる）。**Line 4 = このセッション**（コンテキスト・経過・セッションコスト）、**Line 5 = アカウント**（5h 制限・週間制限・モデル別週間制限・extra-usage）。1 行に混ぜていた頃は 7 要素・弱め表示が 7 割で「どこまでが制限の話で、どこからがこのセッションの話か」が読めなかった。区切り記号を足すより意味の境界で行を割る方を選んだ
  - 毎ターン変わるセッション側を上に置いている（制限は数時間〜1 週間単位でしか動かない）
  - **Bedrock / Vertex / Foundry では制限行が出ない**（全要素が Anthropic 限定）。空行は挟まず 4 行のまま
  - `⣿⣶ 60%/1M  3h  $4.83` / `⣶ 16% 19:31  week:9% 金 12:00  Fable:39% 土 16:00  extra:$2.14`
- **リセット時刻を覚えて、リセットを跨ぐまで再計算しない**。リセットの時刻は次のリセットまで動かないのに、30 秒ごとの再描画で毎回時刻変換を走らせていた。覚えておくことで**外部コマンドの呼び出しが 1 描画あたり 3 回から 1 回に**減り、レート制限を表示している場合と表示していない場合の描画時間の差（約 8ms）が無くなった。使用率そのものは使うたびに変わるのでそのまま毎回読む
- **Line 3 の `gh:owner/repo` を、Line 2 のパスと一致した成分だけ削るようにした**。同じ文字列が Line 2 の末尾と Line 3 の先頭（縦に最も近い位置）で二度出ていた
  - パス末尾が **`owner/repo` と一致** → `gh:` ごと出さない（ghq レイアウト `~/ghq/github.com/owner/repo`）
  - パス末尾が **repo 名だけ一致** → `gh:owner/` に畳む（`~/dev/<repo>` という最も普通の clone レイアウト）。**末尾の `/` は意図的** — 裸の `gh:owner` は「owner という名の repo」に誤読されるため、`/` が「続きは真上の行の末尾」の標識になる
  - **不一致** → 従来どおり全部出す

  `gh:` を出す理由（ローカルのディレクトリ名と origin のリポジトリ名の食い違いはここでしか判別できない）は変えず、**その理由が効くときだけ出す**形にした — 出ているときは必ず何かが違う。大文字小文字が違えば一致しないので「出続ける」側に倒れ、誤って消えることはない。worktree ではパス末尾が worktree 名になるので従来どおり出る
- **リセットを「何時まで」の絶対時刻に統一**。5h は時刻のみ（`19:31`）、週間は曜日付き（`土 16:00`）で、曜日の有無が両者の区別になる。従来 5h は残り時間（`3:31`）だった
  - **既に過ぎているリセットは `now`** と出す（5h・週間の両方）。過ぎた時刻をそのまま出すと「これからリセット」と読めてしまい、実際は過ぎているのに「あと N 時間」と誤読させる

### Added

- **最新版から遅れているときだけ Version を赤くするようにした**（追いついていればグレー 248 のまま）。赤はこの statusline で既に「注意すべき状態」の色（detached / コンフリクト / 削除行 / behind / コンテキスト 90% 超）なので、新しい色相を増やさずにアラームの強さだけを借りている
  - **最新版は Claude Code 自身が置いたキャッシュから読む** — `<config dir>/cache/changelog.md` の冒頭の `## X.Y.Z` が最新リリース。**ネットワークもキャッシュ書き込みも fork もゼロ**（本来ネットワークが要る「最新版」をローカル読み 1 回で得る）。npm registry を背景取得する案は却下 — Claude Code が既に取ってきた答えがディスクにあるのに 2 本目のネットワークを足す理由が無い
  - **状態を持たない** — 「今の版 vs 最新版」だけで決まるので、更新すれば次の描画で自動的にグレーへ戻る（時間の窓もタイムスタンプも持たない）。書き込みが無いので並走ペインの競合も無い
  - 比較は数値（`2.1.9` < `2.1.10`）。読めない / 形式が変わった / 数値でない成分がある場合は**すべてグレーに落ちる**（誤って「遅れている」と出すより無表示に倒す）
- **進行中の git 操作を Line 3 の先頭に出すようにした**（`rebase 1/3` / `merge` / `cherry-pick` / `revert` / `bisect`）。`HEAD@<sha>` だけでは「sha を直接 checkout した」と「rebase 中」が区別できず、Line 3 が状況を伝えられていなかった
  - rebase は進捗つき（`rebase 2/5`）。進捗が読めないときは `rebase` だけ出す
  - 色は既存の赤（detached / コンフリクトと同じ「特別な git 状態」の類なので、行に色系統を増やさない）
  - 判定は git ディレクトリの中を見るだけ（worktree でも正しい場所を引く）。背景取得の中なので描画のコストは増えない
- **output style を Line 1 に出すようにした**（`default` 以外のときだけ）。応答の挙動を根本から変える設定なのに常設の表示が無く、「あのセッションは explanatory のままだった」に気づく口が無かった。既定では出ないので**出ていること自体が「既定と違う」のシグナル**になる（白 231 — Line 1 に唯一残っていた「色相を持たない」枠。最初に置いた light orchid は Agent 名のピンクとほぼ同色で、実機で見分けが付かなかった）
- **モデル別の週間制限**を追加（`Fable:39% 土 16:00`）。モデル名は Line 1 と同じ配色（Fable なら多色、flat 色のモデルはその色）、`%` は通常輝度、リセット時刻は全体の週間制限と同じ弱め表示
  - stdin の `rate_limits` は `five_hour` と `seven_day` の 2 つだけでモデル別の枠は来ない。`extra:$` 用に既に叩いている `/usage` のレスポンスに入っているので、**追加のネットワーク取得も fork も発生しない**（背景取得は 1 回のまま）
  - 全体の週間制限（`week:`）と**併存**する — 別の制限なので置き換えない
  - 枠が複数ある場合は全部出す。1 枠の形式不正では**その行だけ落ちて**他の枠と `extra:$` は生き残る
  - リセット時刻は背景取得の側で表示文字列まで作るので、描画は fork ゼロ（枠ごとに `date` を呼ぶと 30 秒ごとの再描画で毎回 fork が乗る）。取得元が毎回わずかに違う時刻を返して表示分が揺れるため、分単位に丸めて固定している
  - エンドポイントは非公式なので、形式が変わったら枠だけ消えて他は出る（graceful degradation）
  - **取得に失敗したときは前回の値を保つ**（オフラインや応答が遅いときに `extra:$` と枠が消えないように）。モデル名に改行などが混ざっても枠の桁がずれない
- Line 1 のプラン表示を**公式表記＋レート枠**にした（`Anthropic(max)` → `Anthropic(Max 5x)`）。公式プラン名は Free / Pro / **Max 5x** / **Max 20x** / Team / Enterprise で、`subscriptionType` の生値は小文字なのでこれに畳む
- レート枠（`5x` / `20x`）は Keychain の `rateLimitTier` から取る。**値を列挙せず suffix が `Nx` の形かだけを見る** ので、未知の枠（`50x`）や prefix の変更でも動き、`Nx` を持たない値（Pro 相当の `default_claude_ai`）では枠を出さない
- 枠は契約種別と**独立**に付くので、Enterprise / Team 契約でもレート枠が分かる（`Anthropic(Enterprise 5x)`）。実測で Enterprise 契約が `default_claude_max_5x` を持つ
- 取得は既存の背景 fetch に相乗り（`subscriptionType` と `rateLimitTier` を**1 回の jq** で取得）なので、Keychain 読みも fork も増えない。キャッシュは US 区切りの 2 フィールドになったが、旧形式（1 フィールド）を読んでも契約名だけ出て枠が空になるだけなのでファイル名の版は上げていない（最大 3600s で自然回復）
- Built against を Claude Code 2.1.233 に追従。契約面の変更なし — 2.1.232 / 2.1.233 の全 69 エントリに statusLine / subagentStatusLine の記述が無く、stdin JSON も docs と一致している（今回使い始めた `output_style.name` も docs 記載のフィールド）

### Removed

- Line 3 の `base:`（ブランチの切り元）を撤去。reflog の最初の `branch: Created from` を読むので**作成時点としては正しく、現在の基準としては誤る**表示だった — `origin/main` から作ったブランチを後で `origin/develop` に reset すると、`base:origin/main` を出し続ける。開発中に必要なのは「どこへ戻すか」で、それは Line 2 の `from:` が持っている。worktree 以外では切り元の表示が無くなるが、誤読させるより出さないほうを選んだ
- 上記と変更行数への移行でキャッシュのレコード構成が変わったため、キャッシュのファイル名を `-v2` から `-v4` へ。旧キャッシュを読んで桁がずれることはない
...
- 背景セッション（`kind:bg`、`claude agents` 経由）と `/rename <名前>` したセッションで宛名が表示されない問題を修正（1.69.0 の回帰）。宛名は `nameSource` で絞り込まず**常に表示**する。`name` は生成規則にかかわらず `SendMessage` のアドレスなので、絞ると「送れる宛先が画面のどこにも無い」状態が生まれる。`name` の生成規則は 5 通り実測: 素の interactive は `nameSource:derived` + cwd 由来、**背景セッションはキー無し + AI 生成タイトル**（タイトル生成前は 8 桁 job id）、`/branch <名前>` と **`/rename <名前>`** はキー無し + ユーザーが渡した名前。1.69.0 で `"nameSource":"derived"` の明示だけを受ける許可リストにしたのは `/branch <名前>` の二重表示を避ける意図だったが、**キー不在は `/branch <名前>` 固有ではなく**、背景セッションと（interactive で誰でも使う）`/rename` でも宛名が丸ごと消えていた（テストも stderr も赤くならない沈黙した破綻）。`ListAgents` が背景セッションを同じ `name` で列挙することを実測で確認し、往復送信でも検証済み。enum を増やして絞り直すのは同じ破綻を招くので採らない
- 名前に含まれる `"` で宛名が切れて**誤った宛先を表示する**問題を修正。値の中の `"` は JSON では `\"` なので、終端を探す前に `\\` → `\"` の順で退避する（`fix "foo" bug` が `fix \` になっていた）。名前が cwd 由来の slug だけだった頃は踏まない経路だったが、任意タイトルを受けるようになって常用経路に乗った。制御文字の escape（`\n` / ESC）は**意図的に decode しない** — decode すると単一 printf の行数契約が壊れ、AI 生成タイトルからの ANSI 注入経路にもなる。行数がちょうど 4 であることまで含めてテストで固定した
