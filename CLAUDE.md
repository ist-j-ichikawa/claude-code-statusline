...
- **Line 4 (provider-aware)**: Anthropic = rate limit from stdin JSON `rate_limits` field (CC 2.1.80+). `resets_at` is Unix epoch seconds (not ISO 8601). 5-hour in Anthropic sand (`ANTH`, `38;5;180`) with remaining time (`format_reset_remaining`), weekly in dim with absolute day/time reset (`format_reset_absolute`). Pre-2.1.80 CC では `rate_limits` がないため Line 4 は空（graceful degradation）。Bedrock/Vertex/Foundry = session cost (`cost.total_cost_usd`) and token count (`total_input_tokens` + `total_output_tokens`) from stdin JSON.
