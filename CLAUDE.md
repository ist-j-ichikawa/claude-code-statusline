...
- **Effort & thinking (Line 1)**: `.effort.level` (CC 2.1.119+) を `effort:high` 形式、`.thinking.enabled=true` を `think` で表示し、両方ある時は `·` 区切り (`effort:high·think`)。色は**モデル色との衝突回避**のため独立カラー: `EFFORT` (`38;5;105` light purple)、`THINK` (`38;5;117` light cyan)。CCがネイティブのeffort表示を廃止したため統合。レベル severity は文字 (`low`/`high`/`max`) で読み分け、色は単色固定。`effort` キーが欠落した古いCCでも graceful degradation。
- **No width adaptation**: 端末幅に応じた表示切替はしない方針。すべての要素を常時フル表示する。ターミナルが折り返す場合はそれで良しとする（シンプルさ優先）。
- **Line 2 (path + worktree)**: path (OSC 8 クリック可能リンク) + `added_dirs` インジケータ + worktree 🌲。git info は Line 2 から独立して Line 3 に分離済み（Line 2 が長くなりすぎる問題への対処）。
- **Line 3 (git info)**: `build_git()` shows branch name in Git brand orange (`GIT`, `38;5;202`, Pantone 1788C), dirty state (A=staged/M=modified/?=untracked/U=conflicts — git standard symbols), ahead/behind, last commit age+message (20char truncated), detached HEAD (red). Non-git dirs show `(no git)`. Cold start (no cache yet) reads `.git/HEAD` directly (pure bash, no fork) to show branch name immediately; worktree `.git` files with relative gitdir paths are resolved. 専用行にすることでパスの長さに関係なく git 情報が完全表示される。
...
- **README の Line 説明は 2 箇所重複**: `README.md` 「表示レイアウト」(~L14-19) と「スクリプト構造」(~L88-93) で Line 1-4 の役割を別々に記述している。表示変更時は**両方**更新しないと食い違う。
