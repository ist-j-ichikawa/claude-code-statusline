...
- **Model display**: Prefers `display_name`, falls back to `model_id` when empty (e.g. Bedrock). Colors match Anthropic's official brand: Opus=coral(`CORAL`), Sonnet 4.6=teal(`TEAL`), Sonnet 4.5/3.5=amber(`AMBER`), Haiku=lavender(`LAVENDER`)。Fable (Mythos-class, CC 2.1.170+)=steel blue(`FABLE` = `38;5;74`) — 公式ブランド色未発表のため発表ブログのアートワーク（蝶の標本画）の青から導出した暫定色。claude.ai の UI に公式色が現れたら追従する。
