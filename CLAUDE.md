...
- **Model display**: Prefers `display_name`, falls back to `model_id` when empty (e.g. Bedrock). Colors match Anthropic's official brand: Opus=coral(`CORAL`), Sonnet 4.6=teal(`TEAL`), Sonnet 4.5/3.5=amber(`AMBER`), Haiku=lavender(`LAVENDER`)。Fable (Mythos-class, Claude Code 2.1.170+)=steel blue(`FABLE` = `38;5;74`) — 公式ブランド色未発表のため発表ブログのアートワーク（蝶の標本画）の青から導出した暫定色。claude.ai の UI に公式色が現れたら追従する。
- **Effort & thinking (Line 1)**: `.effort.level` (Claude Code 2.1.119+) をレベル名そのまま (`high` / `max` 等)、`.thinking.enabled=true` を `think` で表示し、両方ある時は半角スペース区切り (`high think`)。色は**モデル色との衝突回避**のため独立カラー: `EFFORT` (`38;5;105` light purple)、`THINK` (`38;5;117` light cyan)。Claude Code がネイティブのeffort表示を廃止したため統合。プレフィックスと記号区切りは付けない — 色分けで識別する方針。`effort` キーが欠落した古いCCでも graceful degradation。
- **Vim mode badge (Line 1 最左)**: `.vim.mode` を bg 色付きバッジで Line 1 の**先頭**に表示。`INSERT` = lime-green bg (`VIM_INSERT` = `1;30;48;5;148`、gruvbox light green)、`VISUAL` / `VISUAL LINE` = gold bg (`VIM_VISUAL` = `1;30;48;5;214`、gruvbox yellow、`VISUAL LINE` は `V-LINE` に短縮)。**NORMAL とフィールド欠落時は非表示** (デフォルト状態のノイズ削減 + vim mode 無効セッションの graceful degradation)。Claude Code 組み込みフッターの `-- INSERT --` 表示は dim テキストで見落とされやすいため、bg 色 + bold + 最左配置で**意図的に圧倒的に目立たせる**設計。色は gruvbox / vim-airline の慣例に追従。**自前描画するので `settings.json` の `statusLine.hideVimModeIndicator: true` を推奨** — 組み込みの dim な `-- INSERT --` を抑止し、二重表示を消す（docs が "Set this to true when your script renders `vim.mode` itself" と明記。この設定は CHANGELOG に載らず docs のみ）。自前バッジは残るので mode 表示が失われることはない。
- **No width adaptation**: 端末幅に応じた表示切替はしない方針。すべての要素を常時フル表示する。ターミナルが折り返す場合はそれで良しとする（シンプルさ優先）。
- **Line 2 (path + worktree)**: path (OSC 8 クリック可能リンク) → worktree 🌲 (`from:original_branch` 付き) → `added_dirs` インジケータの順。🌲 は「このパスが worktree である」事実を示すので path 直後、`(+N dirs)` は独立した補助情報なので末尾。git info は Line 2 から独立して Line 3 に分離済み（Line 2 が長くなりすぎる問題への対処）。表示パスは `workspace.current_dir` を使う（`worktree.path` がある場合は上書き済み、`/cd` も Claude Code 2.1.176+ で追従）。**`workspace.project_dir` は使わない** — launch 時ディレクトリに固定され、`/cd` 後に古いパスを出す・worktree 上書きを打ち消す問題があったため v1.32.0 で current_dir に統一（旧 v1.0.0 は `${project_dir:-$current_dir}` で project_dir 優先だった）。
- **Line 3 (git info)**: `build_git()` shows `gh:owner/repo` (dim, GitHub origin only — "GitHub にあげたっけ" の即答用), branch name in Git brand orange (`GIT`, `38;5;202`, Pantone 1788C), **PR review_state** (Claude Code 2.1.145+ `pr.review_state` を テキストで色付き表示: `approved`=緑/`changes_requested`=赤/`pending`=黄/`commented` 他=dim — **PR 番号と URL は Claude Code 組み込みフッターの PR badge と完全に住み分け、こちらは state のみ提供**), `from:<parent>` (dim, reflog `branch: Created from` パース — 切った元ブランチ), dirty state (A=staged/M=modified/?=untracked/U=conflicts — git standard symbols), ahead/behind, last commit age+message (20char truncated), detached HEAD (red). Non-git dirs show `(no git)`. Cold start (no cache yet) reads `.git/HEAD` directly (pure bash, no fork) to show branch name immediately; `.invalid` (Git の空リポ HEAD placeholder、RFC 6761 予約名 — `git init` 直後 / clone 失敗残骸) は dim の `(empty)` に翻訳; Claude Code 2.1.145+ では `workspace.repo` が stdin に来るので cold start でも `gh:` を即表示できる (2.1.144 以前は cache populate 後の 5s 後表示)。worktree `.git` files with relative gitdir paths are resolved. `from:` は reflog が GC される (~90日) と消える / `from:HEAD` (=匿名 HEAD から作成) や clone 直後のブランチでは表示しない。`gh:owner/repo` は **2.1.145+ で `workspace.repo.{host,owner,name}` を優先** (fork ゼロ)、無ければ `git remote get-url origin` を SSH/HTTPS 両形式から正規化（既存の tree URL リンクと共通ロジック）に fallback、non-GitHub remote (GitLab 等) や origin 未設定では表示しない。public/private (visibility) は GitHub API 依存になるので未対応 (gh CLI の active account が `gh auth switch` で切り替わるとリポ毎に false negative を出すため意図的に避けている)。専用行にすることでパスの長さに関係なく git 情報が完全表示される。
- **Worktree indicator (Line 2)**: Shows 🌲 when `worktree.name` (Claude Code worktree session) or `workspace.git_worktree` (git linked worktree, Claude Code 2.1.97+) is set. `from:original_branch` (dim) is appended when `worktree.original_branch` is available (absent for hook-based worktrees). `worktree.path` overrides `current_dir` so that path display and `build_git()` reference the worktree directory, not the original repo.
- **Added dirs (Line 2)**: `workspace.added_dirs` (Claude Code 2.1.78+) の配列長を `(+N dirs)` で表示。`/add-dir` でディレクトリを追加した場合のみ出現。
- **OSC 8 links (Line 2)**: `editor_url` で `file://` URL を生成し、パスをクリック可能にする。カスタムURLスキーム（`zed://` 等）はターミナルが対応しないため `file://` 固定。
- **OSC 8 links (Line 3 branch)**: `build_git()` 内でブランチ名を `${remote}/tree/${branch}` の OSC 8 リンクにする。**Claude Code 組み込みのフッター PR badge** (`prUrlTemplate` でカスタマイズ可、PR状態dot + クリックリンク) と重複しないよう、tree URL のみ提供（PR への遷移は Claude Code 側に任せる）。PRがないブランチでも GitHub に飛べる補完機能として機能。`git remote get-url origin` を SSH/HTTPS 両形式から正規化、non-GitHub remote（GitLab等）と detached HEAD はリンク化スキップ。OSC 8 ラップは既存の `osc8()` ヘルパー再利用。`gh` 依存を意図的に避けてネットワーク呼び出しゼロを維持。Claude Code 2.1.176 の `footerLinksRegexes`（settings 側の機能）は **会話テキスト（Claude のターン出力）** に regex マッチしてフッター行にリンクバッジを出す機能で、公式 docs（statusline.md）に「independent of the status line and do not interact with your script」と明記されており **statusline スクリプト出力には一切適用されない**。`prUrlTemplate` PR badge と同じくフッター行レイヤーなので、自前 OSC 8 リンク（path / tree URL / `gh:`）との競合・重複はなく、スクリプト改修も不要（issue #2 で検証済み）。
- **Upstream tracking**: `~/ghq/github.com/anthropics/claude-code/CHANGELOG.md` で Claude Code の変更を確認。公開リポにソースコードはなく、CHANGELOG + plugins + scripts のみ。`/check-claude-code-update` skill (`.claude/skills/check-claude-code-update/`) が前回チェック済みハッシュを `.claude/upstream-last-checked` に保存し、差分から statusline 影響を分析する正規ワークフロー。**CHANGELOG は網羅的でない** — statusline 設定/フィールドが docs にだけ載ることがある（例: `hideVimModeIndicator` は CHANGELOG 0 件・docs のみ）。版追従時は skill の docs 突き合わせ（Step 2.5）で statusline.md / settings.md も当たる。
- **Line 4 (rate limit + context + cost)**: 左から順に: 5h rate limit (`braille_bar` + %) → context window (`braille_bar` 5-char, `⣀⣄⣤⣦⣶⣷⣿`, 40 steps + %) → weekly rate limit → session cost。5h rate limitを最左に配置（最も頻繁に確認する情報）。Anthropic rate limit は stdin JSON `rate_limits` field (Claude Code 2.1.80+)。`resets_at` is Unix epoch seconds (not ISO 8601). 5-hour in Anthropic sand (`ANTH`, `38;5;180`) with remaining time (`format_reset_remaining`), weekly in dim with absolute day/time reset (`format_reset_absolute`). Pre-2.1.80 Claude Code では rate limit 部分が空（graceful degradation）。**Session cost**: `cost.total_cost_usd` (Claude Code が cache 区分込みで計算済み) を dim の `$X.XX` で最右表示。subscription では実請求なしの参考値。jq 側で `* 100 | round` してセント整数で受け、bash float 演算を回避（fork ゼロ）。`$0.00` とフィールド欠落（旧 Claude Code）は非表示。円換算は為替レート入手がネットワーク呼び出しゼロ方針と衝突するため不採用。トークン数は非表示（Claude Code の `total_input_tokens` がキャッシュトークンを含まず誤解を招くため）。
...
- **jq抽出とコードの同期**: 表示コードを削除/無効化したら、jqの`@sh`抽出行も必ず削除する。未使用のjq抽出はパフォーマンス劣化の原因になる。**未使用判定は bash 変数名で grep する** — `@sh` の代入先名は JSON フィールド名と異なる（例: `.context_window.context_window_size` → `ctx_window_size`）。
...
- **Line 説明は 4 箇所に分散**: `README.md` の「表示レイアウト」「表示例」(具体的なサンプル出力)、`docs/internals.md` の「スクリプト構造」「カラーテーマ」表で Line 1-4 の役割を別々に記述している (実装詳細は v1.33.0 で `docs/internals.md` へ退避済み。README は導入手順に絞る方針)。表示変更時は **これら 4 箇所すべて** + CLAUDE.md + CHANGELOG + バージョンバッジを更新しないと食い違う。
