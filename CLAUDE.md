...
- **Terminal width adaptation**: `COLUMNS` env var → `tput cols` → default 80. Narrow terminals progressively drop low-priority elements: version (<65), session indicator (<55), subscription type (<45), agent name (<45), model version suffix (<35), git info (<45), weekly rate limit (<70). Paths are truncated to 40% of width via `truncate_path`. `_truncate_bytes` provides a byte-level safety net on all output lines with ANSI escape cleanup.
