...
- **Vim mode indicator (Line 1)**: `vim.mode` from stdin JSON (CC 2.1.84+). `[I]` (green) for INSERT, `[N]` (dim) for NORMAL. Hidden when vim is disabled (field absent) or terminal narrow (<55).
- **Terminal width adaptation**: `COLUMNS` env var → `tput cols` → default 80. Narrow terminals progressively drop low-priority elements: version (<65), session indicator (<55), vim mode (<55), subscription type (<45), agent name (<45), model version suffix (<35), git info (<45), worktree indicator (<45), weekly rate limit (<70). Paths are truncated to 40% of width via `truncate_path`. `_truncate_bytes` provides a byte-level safety net on all output lines with ANSI escape cleanup.
- **Git info (Line 2)**: `build_git()` shows dirty state (+staged/~modified/?untracked/!conflicts), ahead/behind, stash count, last commit age+message (20char truncated), detached HEAD (red). Non-git dirs show `(no git)`. Cold start (no cache yet) reads `.git/HEAD` directly (pure bash, no fork) to show branch name immediately; worktree `.git` files with relative gitdir paths are resolved.
- **Worktree indicator (Line 2)**: Uses `worktree.name` from stdin JSON (no git fork needed). Shows 🌲 when in a worktree session, plus `←original_branch` (dim) when `worktree.original_branch` is available (absent for hook-based worktrees).
...
- **Line 3 (context + cost/tokens + rate limit)**: `braille_bar` (5-char, `⣀⣄⣤⣦⣶⣷⣿`, 40 steps) + context percentage, followed by session cost (`cost.total_cost_usd`) and token counts (`total_input_tokens` + `total_output_tokens`) — displayed for all providers. Anthropic additionally shows rate limit from stdin JSON `rate_limits` field (CC 2.1.80+). `resets_at` is Unix epoch seconds (not ISO 8601). 5-hour in Anthropic sand (`ANTH`, `38;5;180`) with remaining time (`format_reset_remaining`), weekly in dim with absolute day/time reset (`format_reset_absolute`). Pre-2.1.80 CC では rate limit 部分が空（graceful degradation）。
